...
class SimpleSelfAttentionModel(nn.Module):
    ...
    def __init__(self, output_size, embeddings, max_length=60):
        ...
        # output classification layer: projects pooled embedding to classes
...
class MultiHeadAttentionModel(nn.Module):
    ...
    def __init__(self, output_size, embeddings, max_length=60, n_head=3, dropout=0.0):
        super().__init__()
        ...
        # φορτώνουμε τα pretrained embeddings
        embeddings = np.array(embeddings)
        num_embeddings, dim = embeddings.shape
        self.token_embedding_table = nn.Embedding(num_embeddings, dim).from_pretrained(
            torch.Tensor(embeddings), freeze=True
        )
    ...
    def forward(self, x):
        ...
        # 2) residual + multi-head attention
        x = x + self.sa(self.ln1(x))                                       # (B, T, C)

        # 3) residual + feed-forward
        x = x + self.ffwd(self.ln2(x))                                     # (B, T, C)

        # 4) average pooling => sentence embedding
        x = x.mean(dim=1)                                                  # (B, C)
...
class TransformerEncoderModel(nn.Module):
    def __init__(self, output_size, embeddings, max_length=60, n_head=3, n_layer=3):
        ...
        # there are blocks of MultiHeadAttention modules as defined below
                # αποθηκεύουμε λίγα attributes για forward
        self.max_length = max_length
        self.n_head = n_head

        # embeddings (λέξεων + θέσης)
        embeddings = np.array(embeddings)
        num_embeddings, dim = embeddings.shape
        self.token_embedding_table = nn.Embedding(num_embeddings, dim)
        self.token_embedding_table = self.token_embedding_table.from_pretrained(
    ...
    def forward(self, x):
        """
        x: Tensor shape (B, T) με token‑ids (T ≤ self.max_length)
        """
        B, T = x.size()
        # word + position embeddings
        tok_emb = self.token_embedding_table(x)                        # (B, T, dim)
        positions = torch.arange(T, device=x.device).unsqueeze(0)      # (1, T)
        pos_emb = self.position_embedding_table(positions)            # (1, T, dim)
        h = tok_emb + pos_emb                                         # (B, T, dim)

        # περάσμα από όλα τα Transformer blocks
        h = self.blocks(h)                                            # (B, T, dim)
        h = self.ln_f(h)                                              # (B, T, dim)

        # average‑pooling κατά μήκος της ακολουθίας
        h = h.mean(dim=1)                                             # (B, dim)

        # προβολή στις κλάσεις
        logits = self.output(h)                                       # (B, output_size)
        return logits
